...
puppies_can_be_picked=True
boss_fight=False
boss=[pygame.image.load("boss.png"),pygame.image.load("boss_punch.png"),pygame.image.load("boss_jump.png")]
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if menu == "game1":
            ...
            if timeg[0] in range(12, 14):
                screen_color = (60, 179, 113)
            elif timeg[0] in range(14, 16):
                screen_color = (46, 139, 87)
            elif timeg[0] in range(16, 18):
                screen_color = (34, 139, 34)
            elif timeg[0] in range(18, 20):
                screen_color = (0, 100, 0)
            elif timeg[0] in range(20, 22):
                screen_color = (75, 0, 130)
            elif timeg[0] in range(22, 24):
    ...
    if healz > 0:
        if x > zombie_x:
            ...
            if boss_fight != True:
                ...
                screen.blit(boss[0], (zombie_x, zombie_y))
        if x < zombie_x:
            screen.fill(screen_color)
            if boss_fight != True:
        ...
        if y > zombie_y:
            ...
            if boss_fight != True:
                ...
                screen.blit(boss[0], (zombie_x, zombie_y))
            zombie_y += 1
        elif zombie_x == x and menu == "game1":
            screen.blit(boss[1],(zombie_x,zombie_y))
            health -= 1
            zombie_x -= 50
    else:
        Zdrop=random.randint(0,10)
        zombie_x = random.randint(0, 1200)
        zombie_y = random.randint(0, 800)
        healz = 3
        expireance += 10
    if Zdrop == 1:
        catridges += random.randint(5,15)
        dialoge = True
        dialoges("у этого были патроны,"
                 "хорошо что он не воспользовался ими", tablet_sprites[0])
    elif Zdrop == 2:
        food += 3
        dialoge = True
        dialoges("У этого мертвеца была еда...",tablet_sprites[0])
    if expireance >= 40:
        expireance = 0
        lvl += 1
    ...
    if event.type == pygame.K_SPACE and food > 0 and menu == "game1":
        food -= 1
        food_level += 5
    if menu == "game0":
    ...
    if x in range(doge_x-100,doge_x+100)  and y in range(doge_y-100,doge_y+100) and boss_fight == False:
        puppies_picked+=1
        puppies_can_be_picked=False
        doge_x=random.randint(-5000,5000)
        doge_x = random.randint(-5000, -5000)
        healz=500
        puppies_can_be_picked = True
    if puppies_picked == 3:
